...
/******************************************************
 * Modified by DBM Mods
 *
 * This file is created in the hopes to provide a
 * easy to use reference for the various methods in
 * dbm that can be exposed to the various scripts we
 * use.
 *
 * @Note This was an absolute pain to make. don't expect
 * a version to be released with every minor
 * release of DBM
 ******************************************************/
...
namespace DBM.Bot {
  ...
  function  initBot() {
    ...
    this.hasMemberIntents =
      (options.intents & DiscordJS.Intents.FLAGS.GUILD_MEMBERS) !== 0;
  ...
  function  mergeSubCommandIntoCommandData(names, data) {
    ...
    let baseCommand =
      this.applicationCommandData.find((data) => data.name === baseName) ??
      null;
    if (baseCommand === null) {
      baseCommand = {
        name: baseName,
    ...
    for (let i = 0; i < parameters.length; i++) {
      ...
      if (name) {
        if (!existingNames[name]) {
          ...
          PrintError(
            MsgType.DUPLICATE_SLASH_PARAMETER,
            commandName,
            i + 1,
            name
          );
  ...
  function checkForCommandErrors() {
    if (this._textCommandCount > 0 && !this.hasMessageContentIntents) {
      PrintError(
        MsgType.SERVER_MESSAGE_INTENT_REQUIRED,
        this._textCommandCount
      );
    }
    if (
    ...
    if (this.clearUnspecifiedServerCommands()) {
      ...
      for (let i = 0; i < serverIdList.length; i++) {
        this.bot.guilds
          ...
          .catch(function (err) {
            PrintError(
              MsgType.INVALID_SLASH_COMMAND_SERVER_ID,
              serverIdList[i]
            );
          });
      }
...
namespace DBM.Actions {
  const actionsLocation = null;
  const eventsLocation = null;
  const extensionsLocation = null;
  ...
  function generateSelectMenu(select, cache) {
    const selectData = {
      ...
      options: select.options.map((option, index) => {
        ...
        option.value =
          this.evalMessage(option.value, cache) || index.toString();
        return option;
      }),
    };
    return selectData;
  ...
  function generateTextInput(textInput, defaultCustomId, cache) {
    const inputTextData = {
      ...
      minLength:
        parseInt(this.evalMessage(textInput.minLength, cache), 10) ?? 0,
      maxLength:
  ...
  function generateData() {
    return [
      ...
      [
        ...
        2,
        true,
        (arg1) => arg1.type !== 'GUILD_TEXT' && arg1.type !== 'GUILD_VOICE',
      ],
      [
        'channelDelete',
        1,
...
namespace DBM.Audio {
  function checkIfHasDependency(key) {
    if (!Audio.packageJson) {
      const path = require('node:path');
      Audio.packageJson = require('node:fs').readFileSync(
  ...
  class Subscription {
    ...
    constructor(voiceConnection) {
      ...
      this.voiceConnection.on('stateChange', async (_, newState) => {
        if (
          newState.status === Audio.voice.VoiceConnectionStatus.Disconnected
        ) {
          if (
            newState.reason ===
    ...
    async processQueue() {
      ...
      try {
        ...
        if (
          Audio.inlineVolume &&
          typeof resource?.volume?.volume === 'number'
        ) {
  ...
  function connectToVoice(voiceChannel) {
    ...
    inlineVolume ??= (Files.data.settings.mutableVolume ?? 'true') === 'true';
